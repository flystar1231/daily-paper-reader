...
title_zh: 基于扩散潜在波束搜索的推理时文本到视频对齐
...
score: 8.0
evidence: 讨论了哪些指标可用于优化视频生成对齐
tldr: 针对文本到视频扩散模型中生成视频内容存在的不自然运动、变形等问题，本文提出了一种推理时的对齐方法。该方法使用扩散潜在波束搜索与前瞻估计器，在生成过程中选择更优的潜变量以最大化给定的对齐奖励。实验表明，该方法能有效提升视频生成的质量和与文本的一致性。该工作为视频生成中的指标优化提供了新思路。
...
motivation: 现有文本到视频模型生成的视频常出现运动不自然、反向播放等问题，需要优化对齐指标。
method: 提出扩散潜在波束搜索与前瞻估计器，在推理时选择更好的扩散潜变量以最大化对齐奖励。
result: 该方法有效提升了视频生成质量和对齐度，优于基线方法。
conclusion: 推理时对齐是提升视频生成质量的有效手段，指标选择至关重要。
---
