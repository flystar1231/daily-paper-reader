...
score: 8.0
evidence: VideoReward：视频生成质量的多维奖励模型
tldr: 论文针对视频生成模型存在的运动不流畅、图文不对齐等问题，构建了大规模人类偏好数据集，并提出了多维奖励模型VideoReward，用于评估视频生成质量。通过强化学习将奖励模型用于模型对齐，显著提升了生成质量。该工作直接为视频生成评估提供了可用的度量工具，与用户对评估指标的需求高度契合。
...
motivation: 现有视频生成模型存在运动不流畅和图文不对齐，需要有效的质量评估工具。
method: 构建人类偏好数据集，训练多维视频奖励模型VideoReward，并用强化学习对齐生成模型。
result: 使用VideoReward对齐的模型在多项指标上优于基线。
conclusion: VideoReward可作为视频生成质量评估的通用指标，直接满足用户对评估度量的需求。
---
