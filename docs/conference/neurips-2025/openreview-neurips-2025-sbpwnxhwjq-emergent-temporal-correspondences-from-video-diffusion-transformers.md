...
score: 8.0
evidence: 提出了用于视频扩散变换器的新的评估指标
tldr: 本文针对视频扩散变换器（DiTs）中时间对应如何建立的问题，提出DiffTrack定量分析框架。该框架构建提示生成视频数据集并引入新评估指标，系统分析3D注意机制中每个组件（表示、层、时间步）对时间对应的贡献。分析揭示了查询-键相似性的关键作用，为理解生成视频时间一致性提供了新视角。
...
motivation: 现有视频扩散模型虽能生成时间一致视频，但缺乏对内部时间对应建立机制的理解。
method: 提出DiffTrack框架，包括构建提示生成视频数据集和伪真实跟踪标注，以及设计评估指标来分析3D注意机制。
result: 实验揭示了查询-键相似性在3D注意中的关键作用，不同层和时间步贡献不同。
conclusion: 该框架为理解和改进视频扩散模型的时间一致性提供了新工具。
---
