...
title_zh: MJ-Video：通过细粒度视频偏好进行视频生成的基准测试与奖励建模
...
score: 9.0
evidence: 提出了一个细粒度视频生成评估的基准和奖励模型
tldr: 针对视频生成模型在指令对齐、幻觉、安全性和偏见等方面的不足，本文构建了MJ-BENCH-VIDEO基准，涵盖对齐、安全、精细度、连贯一致性和偏见公平性五个关键维度及28条细粒度标准。基于此数据集，提出了MJ-VIDEO混合专家视频奖励模型，用于优化生成质量和偏好对齐。该工作为视频生成评估提供了系统化的指标体系和可训练的奖励模型。
...
motivation: 现有视频生成模型存在指令对齐差、幻觉、安全风险和偏见等问题，缺乏细粒度评估体系。
method: 构建了包含28条细粒度标准的大规模视频偏好基准，并提出基于混合专家的视频奖励模型。
result: 所提基准和奖励模型能有效评估和区分视频生成模型的性能，在偏好学习任务中表现出色。
conclusion: 细粒度的视频评估基准和奖励模型是提升视频生成质量的关键工具。
---
